...
const messagesForHumanReview = (filter = state.humanReview.filter) => state.messages.filter((message) => (
  filter === "HUMAN_REVIEW"
    ? categoryFor(message) === "HUMAN_REVIEW"
    : message.documentWorkflowStatus === filter
));

function normalizeHumanReviewSelection() {
  const messages = messagesForHumanReview();
  const totalPages = Math.max(1, Math.ceil(messages.length / PAGE_SIZE));
  state.humanReview.page = Math.min(Math.max(1, state.humanReview.page), totalPages);
  if (!messages.some((message) => message.id === state.humanReview.selectedId)) {
    state.humanReview.selectedId = messages[(state.humanReview.page - 1) * PAGE_SIZE]?.id || messages[0]?.id || null;
  }
}
...
function renderComparison() {
  ...
  const draftPanel = draftIsOpen ? `
      <div class="bl-draft-panel">
        ...
        <div class="bl-draft-actions">
          ...
          <button class="secondary-button" data-bl-download type="button" ${state.blDraft.busy ? "disabled" : ""}>Download file</button>
          <button class="primary-button" data-bl-compose type="button" ${state.blDraft.busy ? "disabled" : ""}>Compose email in Gmail</button>
        </div>
  ...
  comparisonDetail.innerHTML = `
    ...
    <section class="comparison-review-action ${reviewIsComplete ? "complete" : "pending"}">
      <div><strong>${reviewIsComplete ? "Complete" : "Pending"}</strong><p>${reviewIsComplete ? "This case has been reviewed and is filed under Complete." : "Confirm the comparison and Draft BL before marking it complete."}</p></div>
...
function renderHumanReview() {
  ...
  const counts = Object.fromEntries(HUMAN_REVIEW_OPTIONS.map(({ id }) => [id, messagesForHumanReview(id).length]));
  ...
  const messages = messagesForHumanReview();
  const totalPages = Math.max(1, Math.ceil(messages.length / PAGE_SIZE));
  const startIndex = (state.humanReview.page - 1) * PAGE_SIZE;
  const visibleMessages = messages.slice(startIndex, startIndex + PAGE_SIZE);
  const activeLabel = HUMAN_REVIEW_OPTIONS.find(({ id }) => id === state.humanReview.filter)?.label || "Human Review";
  humanReviewList.innerHTML = visibleMessages.length
    ? visibleMessages.map((message) => `
      <button class="mail-row ${message.id === state.humanReview.selectedId ? "selected" : ""}" data-human-message-id="${h(message.id)}" type="button">
        <span class="mail-line"><span class="sender">${h(message.sender)}</span><span class="time">${h(displayTime(message))}</span></span>
        <span class="subject">${h(message.subject)}</span>
        <span class="preview">${h(message.snippet || message.body)}</span>
        <span class="badges"><span class="badge badge-human_review">${h(activeLabel)}</span></span>
      </button>`).join("")
    : `<div class="empty-list">No cases currently require ${h(activeLabel.toLowerCase())} review.</div>`;

  if (messages.length <= PAGE_SIZE) humanReviewPagination.replaceChildren();
  else {
    humanReviewPagination.innerHTML = `
      <button class="pagination-button" data-human-page="previous" ${state.humanReview.page === 1 ? "disabled" : ""}>Previous</button>
      <span class="pagination-info">${startIndex + 1}–${Math.min(startIndex + PAGE_SIZE, messages.length)} of ${messages.length}</span>
      <button class="pagination-button" data-human-page="next" ${state.humanReview.page === totalPages ? "disabled" : ""}>Next</button>`;
  }
  ...
  if (isIntentReview) {
    actionMarkup = `<section class="human-review-action">
      ...
      <div class="intent-label-actions">
        ${INTENT_RESOLUTION_CATEGORIES.map((category) => `<button class="intent-label-button" data-resolve-category="${category.id}" type="button" ${state.humanReview.busy ? "disabled" : ""}>${h(category.label)}</button>`).join("")}
      </div>
  ...
  humanReviewDetail.innerHTML = `
    <div class="case-header">
      <div class="case-title-row"><div><span class="eyebrow">${h(activeLabel)}</span><h1>${h(message.subject)}</h1></div><button class="view-gmail-button" data-human-view-gmail type="button">View Email in Gmail</button></div>
      <p>${h(message.senderAddress)} · ${h(displayTime(message))}</p>
    </div>
    <div class="classification-insight review"><div><strong>${h(activeLabel)}</strong><span>${h(`${message.confidence ?? 0}% confidence`)}</span></div><p>${h(isIntentReview ? message.classificationReason || message.reviewReason || "The email intent needs confirmation." : message.documentReviewReason || "The document issue needs manual review.")}</p></div>
    <div class="detail-body">
      <section class="message-content"><h2>Email content</h2><p>${h(message.body)}</p></section>
      <section class="attachments-section"><div class="section-heading"><h2>Files</h2><span>${attachments.length}</span></div><div class="attachment-grid">${attachmentCards}</div></section>
    </div>
...
humanReviewPagination.addEventListener("click", (event) => {
  const button = event.target.closest("[data-human-page]");
  if (!button || button.disabled) return;
  const messages = messagesForHumanReview();
  const totalPages = Math.max(1, Math.ceil(messages.length / PAGE_SIZE));
  state.humanReview.page = button.dataset.humanPage === "next"
    ? Math.min(totalPages, state.humanReview.page + 1)
    : Math.max(1, state.humanReview.page - 1);
  state.humanReview.selectedId = messages[(state.humanReview.page - 1) * PAGE_SIZE]?.id || null;
  humanReviewList.scrollTop = 0;
  renderHumanReview();
});
