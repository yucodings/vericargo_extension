...
const suspiciousWord = (word) => {
  ...
  return longestConsonantRun(lower) >= 7 || vowelRatio < 0.14 || vowelRatio > 0.8;
};

export function findDraftContentIssues(text) {
  const issues = [];
  ...
  String(text || "").split(/\r?\n/).forEach((line, index) => {
    ...
    const suspiciousWords = words.filter(suspiciousWord);
    ...
    issues.push({
      excerpt: trimmed.slice(0, 80),
      line: index + 1,
      reason: suspiciousWords.length
        ? "This line contains text that does not resemble a meaningful word or shipping identifier."
        : "This line contains an unusually large sequence of symbols.",
    });
  });
...
export function createBlDocument(message, requestedFormat = "pdf", editedText) {
  ...
  const contentIssues = findDraftContentIssues(text);
  if (contentIssues.length) {
    const firstIssue = contentIssues[0];
    const error = new Error(`Possible gibberish detected on line ${firstIssue.line}. Correct or remove it before creating a Draft BL file.`);
