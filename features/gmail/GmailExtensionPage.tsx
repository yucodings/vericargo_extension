    ...
    <main className="min-h-screen bg-[#edf3f8] p-2 text-[#15233b] sm:p-4">
      <header className="mx-auto mb-3 flex max-w-[1800px] items-center gap-3 rounded-2xl border border-[#d6e0e9] bg-white px-3 py-3 shadow-[0_10px_30px_rgba(29,54,86,0.07)] sm:px-5">
        <button className="focus-ring flex shrink-0 items-center gap-3 rounded-xl" onClick={() => setSelectedId(null)} aria-label="Show inbox overview">
          ...
          <span className="hidden sm:block"><strong className="block text-lg tracking-[0.12em] text-[#123b78]">BLiNK</strong><small className="block text-xs text-slate-500">Gmail shipping assistant</small></span>
        </button>
      ...
      <div className="mx-auto grid max-w-[1800px] gap-3 xl:h-[calc(100vh-105px)] xl:grid-cols-[350px_minmax(420px,1fr)_430px]">
        ...
        <EmailPanel item={selected} onClose={() => setSelectedId(null)} onCompare={() => setNotice("The latest processing run is already reflected in the BLiNK panel.")} />
...
function CategoryBadge({ category }: { category: EmailCategory }) { const value = categoryPresentation[category]; return <Badge variant="outline" className={value.className}>BLiNK/{value.label}</Badge>; }
